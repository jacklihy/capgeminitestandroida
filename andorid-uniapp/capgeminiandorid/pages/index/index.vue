<template>
    <view class="index">
	<musichead title="图书管理系统" :icon="false"></musichead>
	<view class="container">
		<scroll-view scroll-y="true">
			<view class="index-search">
				<text class="iconfont icon-fangdajing"></text>
				<input  type="text" placeholder="搜索图书" @tap="handleToSearch"/>
			</view>
			<view v-if="isLaoding">
				 <m-for-skeleton
						:avatarSize="200"
...
<script>
	...
	export default {
		...
		onLoad() {
			topList().then((res)=>{
				if(res.length){
					setTimeout(()=>{this.topList=res;},1000);
...
<style scoped>
	 .index{}
	 .index-search{ display: flex; align-items: center;height: 70rpx; margin: 70rpx 30rpx 30rpx 30rpx;
	 ...
	 .index-list-img text{ position: absolute; left:12rpx; bottom: 16rpx; color: white; 
	 font-size:20rpx; }
